...
def get_available_services() -> dict[str, bool | None]:
    ...
    if confluence_url:
        ...
        # Check for minimal OAuth (user-provided tokens) when URL is set but no credentials
    ...
    if jira_url:
        ...
        # Check for minimal OAuth (user-provided tokens) when URL is set but no credentials
        if not jira_is_setup and os.getenv("ATLASSIAN_OAUTH_ENABLE", "").lower() in ("true", "1", "yes"):
            jira_is_setup = True
            logger.info(

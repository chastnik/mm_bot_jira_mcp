...
sequenceDiagram
    ...
    User->>MM: Отправляет сообщение
    MM->>Bot: WebSocket событие "posted"
    Bot->>Handler: handle_message()
    
    alt Нет учетных данных
        Handler->>Storage: has_jira_credentials()
        Storage-->>Handler: false
        Handler->>User: Запрос учетных данных
        User->>Handler: Логин и пароль
        Handler->>Auth: save_and_validate_both()
        Auth->>Jira: Проверка учетных данных
        Jira-->>Auth: Успех/Ошибка
        Auth->>Storage: Сохранение (шифрование)
        Storage-->>Auth: OK
        Auth-->>Handler: Успех
    end
    
    Handler->>Storage: get_user_credentials()
    Storage-->>Handler: Расшифрованные данные
    
    Handler->>MCP: list_tools(auth_headers)
    MCP->>MCPServer: HTTP запрос с Basic Auth
    MCPServer-->>MCP: Список инструментов
    MCP-->>Handler: Инструменты
    
    Handler->>LLM: chat(messages, tools)
    LLM-->>Handler: Ответ с tool_calls
    
    loop Для каждого tool_call
        Handler->>MCP: call_tool(name, args, auth_headers)
        MCP->>MCPServer: HTTP запрос
        MCPServer->>Jira: API запрос
        Jira-->>MCPServer: Данные
        MCPServer-->>MCP: Результат
        MCP-->>Handler: Результат
        Handler->>LLM: Результат инструмента
        LLM-->>Handler: Следующий ответ
    end
    
    Handler->>Storage: Сохранение в историю
    Handler->>MM: Ответ пользователю
    MM->>User: Сообщение от бота
```
...
stateDiagram-v2
    ...
    Проверка_учетных_данных --> Нет_данных: Учетные данные отсутствуют
    Проверка_учетных_данных --> Есть_данные: Учетные данные есть
    
    Нет_данных --> Запрос_логина: Начало регистрации
    Запрос_логина --> Запрос_пароля: Получен логин
    Запрос_пароля --> Валидация: Получен пароль
    
    Валидация --> Проверка_Jira: Валидация учетных данных
    Проверка_Jira --> Проверка_Confluence: Jira OK
    Проверка_Jira --> Ошибка_Jira: Jira ошибка
    
    Проверка_Confluence --> Сохранение: Confluence OK (или не настроен)
    Проверка_Confluence --> Ошибка_Confluence: Confluence ошибка
    
    Ошибка_Jira --> Запрос_логина: Повтор
    Ошибка_Confluence --> Запрос_логина: Повтор
    
    Сохранение --> Шифрование: Шифрование паролей
    Шифрование --> Сохранение_в_БД: Сохранение в SQLite
    Сохранение_в_БД --> Есть_данные: Успех
    
    Есть_данные --> Обработка_запроса: Готов к работе
    Обработка_запроса --> [*]
```
...
graph LR
    ...
    JiraMixin -->|9. Результат| MCPServer
    MCPServer -->|HTTP Response| MCPClient
    MCPClient -->|10. Результат| Handler
```
...
graph TB
    subgraph "Инициализация"
        Start[Запуск бота]
        CheckKey{Ключ<br/>существует?}
        GenKey[Генерация ключа<br/>Fernet.generate_key]
        SaveKey[Сохранение в<br/>.encryption_key]
        LoadKey[Загрузка ключа]
    end
    
    subgraph "Шифрование"
        PlainText[Пароль пользователя]
        KDF[PBKDF2HMAC<br/>SHA256, 100k итераций]
        DeriveKey[Деривация ключа<br/>32 байта]
        Fernet[Fernet Cipher]
        Encrypted[Зашифрованный текст<br/>Base64]
    end
    
    subgraph "Хранение"
        SQLite[(SQLite Database<br/>bot_data.db)]
        Table[Таблица users:<br/>- mm_user_id<br/>- jira_username<br/>- jira_password_encrypted<br/>- confluence_username<br/>- confluence_password_encrypted]
    end
...
Для остановки бота используйте скрипт `stop_bot.sh`:

```bash
./scripts/stop_bot.sh
```

**Что делает скрипт `stop_bot.sh`:**
- Мягко завершает процессы `mattermost-bot` и `mcp-atlassian` (SIGTERM)
- Освобождает порт MCP сервера (по умолчанию 8000)
- При необходимости применяет принудительное завершение (SIGKILL)
- Выполняет финальную проверку и завершает оставшиеся процессы

**Примечание:** Скрипт автоматически определяет порт из переменной окружения `PORT` (по умолчанию 8000).
...
```mermaid
graph LR
    A[Пароль пользователя] --> B[PBKDF2HMAC<br/>SHA256<br/>100,000 итераций]
    B --> C[Деривация ключа<br/>32 байта]
    C --> D[Fernet Cipher<br/>AES-128 в CBC режиме]
    D --> E[Зашифрованный текст<br/>Base64]
    E --> F[SQLite Database]
    
    G[Ключ шифрования] --> H[.encryption_key<br/>chmod 600]
    H --> B
```
